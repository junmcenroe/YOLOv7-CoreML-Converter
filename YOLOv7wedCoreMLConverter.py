...
def addExportLayerToCoreml(builder,featureMapDimensions,strides,anchorGrid,numberOfClassLabels, imgSize):
    ...
    #P6 model
    builder.set_output(output_names=["raw_confidence", "raw_coordinates"], output_dims=[
                        (int(3*((imgSize/strides[0])**2+(imgSize/strides[1])**2+(imgSize/strides[2])**2+(imgSize/strides[3])**2)),numberOfClassLabels),
                        (int(3*((imgSize/strides[0])**2+(imgSize/strides[1])**2+(imgSize/strides[2])**2+(imgSize/strides[3])**2)),4)])
...
def main():
    ...
    parser.add_argument('-input','--model-input-path', type=str, dest="model_input_path",
                        default='models/yolov5s.mlmodel', help='path to yolov5 export model')
    parser.add_argument('-output','--model-output-directory', type=str,
    ...
    #strides = [8, 16, 32] #for P5 model
    strides = [8, 16, 32, 64] #for P6 model
    if reverseModel:
        strides.reverse()
    ...
    #anchors = ([9, 11, 21, 19, 17, 41], [43, 32, 39, 70, 86, 64], [
    #           65, 131, 134, 130, 120, 265],[282, 180, 247, 354, 512, 387])  # Take these from the <model>.yml in yolov5 P6_640 model
    anchors = ([19, 27, 44, 40, 38, 94], [96, 68, 86, 152, 180, 137], [
               140, 301, 303, 264, 238, 542],[436, 615, 739, 380, 925, 792])  # Take these from the <model>.yml in yolov5 P6_1280 model
    ...
    anchorGrid = torch.tensor(anchors).float().view(4, -1, 1, 1, 2)


    #Use .mlmodel exported using by export.py which need train mode export
